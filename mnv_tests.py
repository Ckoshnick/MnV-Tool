...
def mc_test_single(dk):
    ...
    """
    # INPUT PARAMS
    modelDict = {'var': 'CDH + HDH + C(month) + C(hour)',
...
def mc_test_all_linear(dk):
    ...
    """
    # INPUT PARAMS
    modelDict = {'var': 'CDH + HDH + C(month) + C(weekday)',
                 'testTrainSplit': 'random',
                 'randomState': 4291990,
                 'testSize': 0.2,
                 'commodityRate': 0.056,
...
def test_jupyter(data):
    ...
    modelDict = {'var': 'CDH + HDH + C(daytime) +C(month)',
                 'testTrainSplit': 'random',
                 'randomState': 9285785,
                 'testSize': 0.25,
                 'commodityRate': 0.056,
                 'varPermuteList': ['', 'C(daytime)', 'C(weekday)', 'C(month)']}
    ...
    allmod.plot_pool(0)
    modelDict['var'] = allmod.statsPool.iloc[0]['var']
